...
export default function OrderDetails({
  ...
  const customerCol = showDeliveryTo ? "col-span-3" : "col-span-2";
  ...
  return (
    <div className="border border-slate-200 rounded-lg">
      ...
      >
        ...
        <div
          className={`${customerCol} text-slate-700 ${
            showDeliveryTo ? "whitespace-normal break-words" : "truncate"
          }`}
        >
          {order.Customer}
        </div>
          ...
          <div className={`${deliveryToCol} text-slate-700 flex items-center gap-2`}>
            {isEditingDeliveryTo ? (
              <div className="flex w-full flex-col gap-2">
                <select
                  className="h-8 w-full rounded border border-slate-200 bg-white px-2 text-sm"

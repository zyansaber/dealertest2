...
export default function DealerGroupDashboard() {
  ...
  const scopedLocation = useMemo(
    () => ({
      ...location,
      pathname: `/dealer/${currentDealerSlug}/dashboard`,
    }),
    [location, currentDealerSlug]
  );
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 min-h-screen">
        <Routes location={scopedLocation}>
          <Route path="/dealer/:dealerSlug/dashboard" element={<DealerOverallDashboard key={currentDealerSlug} />} />
        </Routes>
      </main>
    </div>
  );

...
const extractSchedule2Rows = (raw: unknown): ScheduleItem[] => {
  if (!raw) return [];
  if (Array.isArray(raw)) return raw.filter((x) => x && typeof x === "object") as ScheduleItem[];
  if (typeof raw !== "object") return [];
  const rec = raw as Record<string, unknown>;
  if (Array.isArray(rec.data)) return rec.data.filter((x) => x && typeof x === "object") as ScheduleItem[];
...
export default function PlanningIntegration() {
  ...
  useEffect(() => {
    const scheduleRef = ref(database, "schedule/2");
    const handler = (snapshot: any) => {
      setSchedule(extractSchedule2Rows(snapshot.val()));
      setIsLoading(false);
    };
  ...
  const differences = useMemo(() => {
    ...
    trackedMilestones.forEach((m) => {
      ...
      months.forEach((month) => {
        ...
        const target = targets[m]?.[month] ?? 0;
  ...
  const setTarget = (metric: string, month: string, value: string) => {
    const parsed = Number(value);
    setTargets((prev) => ({ ...prev, [metric]: { ...(prev[metric] ?? {}), [month]: Number.isFinite(parsed) ? parsed : 0 } }));
  };
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <main className="ml-72 min-w-0 p-6">
        {activeTab === "planningintegration" && (
          <>
            <div className="mb-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-2xl font-semibold tracking-tight">Planning Integration</h2><p className="mt-1 text-sm text-slate-600">Data source sorting: /schedule/2 order.</p></div>
        ...
        {activeTab === "target" && (
          <>
            <div className="mb-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-2xl font-semibold tracking-tight">Target</h2><p className="mt-1 text-sm text-slate-600">Set monthly targets from 2025-06 and compare Actual - Target.</p></div>
            <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm overflow-auto">
              <table className="min-w-[1400px] text-sm"><thead><tr><th className="px-2 py-2 text-left">Metric</th>{months.map((m) => <th key={m} className="px-2 py-2 text-left">{m}</th>)}</tr></thead><tbody>{trackedMilestones.map((metric) => <tr key={metric} className="border-t"><td className="px-2 py-2 font-medium">{metric}</td>{months.map((month) => <td key={`${metric}-${month}`} className="px-2 py-2"><input type="number" className="w-24 rounded border px-2 py-1" value={targets[metric]?.[month] ?? 0} onChange={(e) => setTarget(metric, month, e.target.value)} /></td>)}</tr>)}</tbody></table>
            </div>
            <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-3">
              <Card><CardHeader><CardTitle className="text-sm">Total Difference (Actual - Target)</CardTitle></CardHeader><CardContent><p className={`text-3xl font-bold ${totalDifference >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{totalDifference}</p></CardContent></Card>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 text-sm font-semibold">Difference Bar Chart: {targetMetric} (Actual - Target)</div>
        ...
        {activeTab === "report" && <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm text-sm text-slate-600">Use target tab for monthly target difference and total difference.</div>}
      </main>
    </div>
  );
}

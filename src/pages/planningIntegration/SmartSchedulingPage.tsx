...
type ScopeKey = "melbourneAndTransit" | "longtreeFactory" | "longtreeWeldingToFinished";

const scopeText = (lang: PlanningLang, scope: ScopeKey) => {
  if (scope === "melbourneAndTransit") return tr(lang, "Melbourne Factory + In Transit", "墨尔本工厂 + 在途运输");
  if (scope === "longtreeFactory") return tr(lang, "Longtree Factory", "Longtree 工厂");
...
export default function SmartSchedulingPage({ rows, lang }: { rows: Row[]; lang: PlanningLang }) {
  ...
  const scopedRows = useMemo(() => {
    const melbourneAndTransitStatuses = new Set([
      "Melbourn Factory",
    ...
    return {
      melbourneAndTransit: enhancedRows.filter((r) => melbourneAndTransitStatuses.has(r.status)),
      longtreeFactory: enhancedRows.filter((r) => longtreeFactoryStatuses.has(r.status)),
      longtreeWeldingToFinished: enhancedRows.filter((r) => longtreeWeldingToFinishedStatuses.has(r.status)),
    };
  }, [enhancedRows]);

  const sections: ScopeKey[] = ["melbourneAndTransit", "longtreeFactory", "longtreeWeldingToFinished"];
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-slate-600">
            {tr(
              lang,
              "Percentages are shown in the same dimensions as overall-dashboard/overview: model range, customer type, and dealer group.",
              "百分比维度与 overall-dashboard/overview 一致：Model Range、Customer（客户/库存）、By Group。"
            )}
          </p>
        </CardHeader>
      </Card>

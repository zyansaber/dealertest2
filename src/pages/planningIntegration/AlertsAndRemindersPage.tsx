  ...
  const reminders = useMemo(() => {
    ...
    return rows
      .map((row) => {
        ...
        const timelineOneLine = points.map((p) => `${p.label}: ${formatDate(p.ts)}`).join("   ");
        ...
        for (const seg of reasonSegmentMap) {
          ...
          if (fromTs != null && toTs == null) {
            ...
            slowReason = `${seg.from} → ${seg.to}${customReason ? `：${customReason}` : lang === "zh" ? "：待补充原因" : ": reason pending"}`;
            break;
          }
        }
                    ...
                    alerts.map((item) => {
                      ...
                      return (
                        <tr key={item.reasonKey} className={`border-t border-slate-100 align-top ${customerRowClass}`}>
                          ...
                          <td className="px-3 py-2"><span className={`rounded-full px-2 py-0.5 text-xs font-semibold uppercase ${item.type === "customer" ? "bg-amber-200/70 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{item.type}</span></td>

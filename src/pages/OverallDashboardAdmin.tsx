...
export default function OverallDashboardAdmin() {
  ...
  const tierGapRows = useMemo(() => {
    ...
    return outlookRows
      .filter((row) => assignedTierByModel[row.model] === selectedTier)
      .flatMap((row) => {
  ...
  }, [activeRule, assignedTierByModel, outlookRows, selectedTier]);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-md border bg-slate-50 p-3 text-sm text-slate-700">
              Gap list只显示已归类到 <strong>{selectedTier.toUpperCase()}</strong> 的Model，并按规则比较：
              <strong>Yard + Incoming</strong> vs <strong>Handover 6m/3m(stock) × 倍率</strong>。
            </div>
                ...
                {tierGapRows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center text-slate-500">
                      No gaps found for {selectedTier.toUpperCase()} rule (check model tier assignments first).
                    </TableCell>
                  </TableRow>
                ) : (

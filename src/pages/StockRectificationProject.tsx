...
export function StockRectificationView({ dealerSlug, showDealerList = true }: StockRectificationViewProps) {
  ...
  const [activeFilter, setActiveFilter] = useState<"all" | "physically" | "missing" | "report-invalid">("all");
  ...
  useEffect(() => {
    ...
    setReasonDrafts((prev) => {
      const next = { ...prev };
      Object.entries(reports).forEach(([chassis, report]) => {
        if (next[chassis] === undefined && report?.reason != null) {
          next[chassis] = String(report.reason);
        }
      });
      return next;
    });
    setCustomReasonDrafts((prev) => {
      const next = { ...prev };
      Object.entries(reports).forEach(([chassis, report]) => {
        if (next[chassis] === undefined && report?.customReason != null) {
          next[chassis] = String(report.customReason);
        }
      });
      return next;
    });
  ...
  useEffect(() => {
    return () => {
      ...
      Object.values(reasonTimersRef.current).forEach((timerId) => window.clearTimeout(timerId));
      reasonTimersRef.current = {};
    };
  }, []);
  ...
  const rows = useMemo(() => {
    ...
    const inventoryKeys = new Set([
      ...pendingRows.map((row) => row.chassis),
      ...stockRows.map((row) => row.chassis),
    ]);

    const reportRows = Object.entries(reports || {}).flatMap(([key, report]) => {
      const chassis = toStr(report?.chassis || key).toUpperCase();
      if (inventoryKeys.has(chassis)) return [];
      return [
        {
          chassis,
          model: "-",
    ...
    return [...pendingRows, ...stockRows, ...reportRows].sort((a, b) => a.chassis.localeCompare(b.chassis));
  }, [yardPending, yardStock, reports]);
  ...
  const handleSaveReason = async (chassis: string, nextReason?: string, nextCustom?: string) => {
    const reason = (nextReason ?? reasonDrafts[chassis] ?? "").trim();
    const customReason = (nextCustom ?? customReasonDrafts[chassis] ?? "").trim();
    const report = reports?.[chassis];
    if (!reason) {
      await updateStockRectificationEntry(currentDealer.slug, chassis, {
        reason: null,
        customReason: null,
      });
      return;
    ...
    await updateStockRectificationEntry(currentDealer.slug, chassis, {
      ...
      source: report?.source ?? "report-invalid-stock",
    });
  };
  ...
  const queueReasonSave = (chassis: string, reason: string, customReason: string) => {
    const existingTimer = reasonTimersRef.current[chassis];
    if (existingTimer) {
      window.clearTimeout(existingTimer);
    }
    reasonTimersRef.current[chassis] = window.setTimeout(() => {
      handleSaveReason(chassis, reason, customReason);
      delete reasonTimersRef.current[chassis];
    }, 600);
  };
  ...
  const handleReportInvalid = async (chassis: string) => {
    try {
      await updateStockRectificationEntry(currentDealer.slug, chassis, { source: "report-invalid-stock" });
    } catch (error) {
      console.error(error);
      toast.error("Failed to report invalid stock.");
    }
  };

  const inventoryKeys = useMemo(() => {
    const keys = new Set<string>();
    Object.entries(yardPending || {}).forEach(([key, value]) => {
      const row = value ?? {};
      const chassis = toStr(row.chassis || key).toUpperCase();
      keys.add(chassis);
    });
    Object.entries(yardStock || {}).forEach(([key, value]) => {
      const row = value ?? {};
      const chassis = toStr(row.chassis || key).toUpperCase();
      keys.add(chassis);
    });
  ...
  const physicallyInYardCount = useMemo(
    () => Object.values(reports || {}).filter((report: any) => report?.physicallyInYard === true).length,
    [reports]
  );

  const physicallyMissingCount = useMemo(
    () =>
      Object.entries(reports || {}).filter(([key, report]) => {
        if (report?.physicallyInYard !== true) return false;
        const chassis = toStr(report?.chassis || key).toUpperCase();
        return !inventoryKeys.has(chassis);
      }).length,
    [reports, inventoryKeys]
  );

  const reportInvalidCount = useMemo(
    () => Object.values(reports || {}).filter((report: any) => report?.source === "report-invalid-stock").length,
    [reports]
  );

  const filteredRows = useMemo(() => {
    if (activeFilter === "physically") {
      return rows.filter((row) => reports?.[row.chassis]?.physicallyInYard === true);
    }
    if (activeFilter === "missing") {
      return rows.filter((row) => row.isMissingFromInventory && reports?.[row.chassis]?.physicallyInYard === true);
    }
    if (activeFilter === "report-invalid") {
      return rows.filter((row) => reports?.[row.chassis]?.source === "report-invalid-stock");
    }
    return rows;
  }, [activeFilter, reports, rows]);

  const rowPriority = (row: typeof rows[number]) => {
    const report = reports?.[row.chassis];
    if (report?.source === "report-invalid-stock") return 1;
    if (report?.source === "add-to-yard") return 2;
    if (row.isPending) return 3;
    if (row.isMissingFromInventory) return 4;
    return 5;
  };
      ...
      <div className="grid gap-6">
        ...
        <Card className="border-slate-200 bg-white">
          ...
          <CardContent className="flex flex-wrap gap-2">
            ...
            <Button
              size="sm"
              variant={activeFilter === "physically" ? "default" : "outline"}
              onClick={() => setActiveFilter("physically")}
            >
              Physically in yard ({physicallyInYardCount})
            </Button>
            <Button
              size="sm"
            ...
            >
              Physically in yard but not in inventory ({physicallyMissingCount})
            </Button>
            <Button
              size="sm"
              variant={activeFilter === "report-invalid" ? "default" : "outline"}
              onClick={() => setActiveFilter("report-invalid")}
            >
              Report invalid ({reportInvalidCount})
            </Button>
          </CardContent>
        </Card>
                ...
                <Table>
                  ...
                  <TableBody>
                    {sortedRows.map((row) => {
                      ...
                      const reportSource = report?.source ?? null;
                      const isReportedInvalid = reportSource === "report-invalid-stock";
                      ...
                      const noteValue =
                        noteDrafts[row.chassis] !== undefined
                          ...
                          : "";
                      const reasonValue =
                        reasonDrafts[row.chassis] !== undefined
                          ? reasonDrafts[row.chassis]
                          : reportedReason || "";
                      const customReasonValue =
                        customReasonDrafts[row.chassis] !== undefined
                          ? customReasonDrafts[row.chassis]
                          : reportedCustom || "";
                      const rowClassName = row.isPending
                        ? "bg-amber-50/70"
                        : reportSource === "report-invalid-stock"
                        ? "bg-rose-50"
                        : reportSource === "add-to-yard"
                        ? "bg-sky-50"
                        : row.isMissingFromInventory
                        ? "bg-violet-50"
                        : undefined;
                      return (
                        <TableRow key={row.chassis} className={rowClassName}>
                          ...
                          <TableCell>
                            {isReportedInvalid ? (
                              <span className="text-xs font-semibold uppercase text-rose-600">Reported</span>
                            ) : (
                              <Button
                                size="sm"
                                variant="outline"
                                className="border-rose-200 text-rose-700 hover:bg-rose-50"
                                onClick={() => handleReportInvalid(row.chassis)}
                              >
                                Report
                              </Button>
                            )}
                          ...
                          <TableCell className="text-slate-700">
                            <div className="space-y-2">
                              <Select
                                ...
                                onValueChange={(value) =>
                                  setReasonDrafts((prev) => {
                                    const next = { ...prev, [row.chassis]: value };
                                    queueReasonSave(row.chassis, next[row.chassis], customReasonValue);
                                    return next;
                                  })
                                }
                              >
                              ...
                              {(reasonValue || reportedReason) === "Other" && (
                                <Input
                                  ...
                                  onChange={(event) =>
                                    setCustomReasonDrafts((prev) => {
                                      ...
                                      queueReasonSave(row.chassis, reasonValue, next[row.chassis]);
                                      return next;
                                    })
                                  }
                                />

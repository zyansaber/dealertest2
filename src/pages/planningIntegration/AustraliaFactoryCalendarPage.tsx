    ...
    <div className="space-y-3">
      <div className="rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700 shadow-sm">
        ...
        <div className="flex flex-wrap gap-4 text-xs">
          <span className="inline-flex items-center gap-2"><span className="h-3 w-3 rounded bg-red-100" /> {lang === "zh" ? "休息日" : "Break Day"}</span>
          <span className="inline-flex items-center gap-2"><span className="h-3 w-3 rounded bg-slate-100" /> {lang === "zh" ? "周末" : "Weekend"}</span>
          <span className="inline-flex items-center gap-2"><span className="h-3 w-3 rounded border border-slate-300 bg-white" /> {lang === "zh" ? "工作日" : "Working Day"}</span>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
        {months.map((m) => (
          <div key={m.month} className={`rounded-2xl border p-3 shadow-sm ${m.isCurrentMonth ? "border-emerald-500 bg-emerald-50/60 ring-1 ring-emerald-200" : "border-slate-200 bg-white"}`}>

  ...
  const outlookRows = useMemo(() => {
    ...
    Object.entries(handoverAll || {}).forEach(([dealerSlug, rows]) => {
      Object.entries(rows || {}).forEach(([key, payload]) => {
        ...
        if (!model) return;
        const handoverDealer = normalizeDealerSlug(rec?.dealerSlug || rec?.dealerName || dealerSlug);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-md border bg-slate-50 p-3 text-sm text-slate-700">
              以下先固定展示 <strong>TIER1 + Factory dealer</strong> 的全部 dealer/model 明细（不是只看 gap），用于排查：

...
export default function SchedulePage({
  ...
  const searchedVehicleOrders = useMemo(() => {
    ...
    return campervanOrders
      ...
      .sort((a, b) =>
        String(a.forecastProductionDate || "").localeCompare(
  ...
  const updateVehicleStatus = async (
    ...
    nextStatus: EditableVehicleStatus,
  ) => {
    const id = order._id;
    if (!id) return;
    try {
              ...
              <table className="min-w-[1500px] text-sm">
                ...
                <tbody>
                  {searchedVehicleOrders.map((order, idx) => {
                    const status =
                      String(
                        order.regentProduction || "longtreeScheduling",
                      ).trim() || "longtreeScheduling";
                    const currentStatus = editableVehicleStatuses.includes(
                        ...
                        <td className="whitespace-nowrap px-3 py-2.5">
                          <select
                            ...
                            onChange={(e) =>
                              updateVehicleStatus(
                                order,
                                e.target.value as EditableVehicleStatus,
                              )
                            }
                            className={`rounded-md border border-slate-300 px-2 py-1.5 text-xs font-semibold ${vehicleStatusClass[currentStatus] || "bg-slate-100 text-slate-700"}`}
                          >
                            {editableVehicleStatuses.map((item) => (
                              <option key={item} value={item}>
                                {lang === "zh"
                                  ? statusText(lang, item)
                                  : vehicleStatusText[item]}
                              </option>
                            ))}
                          </select>
                        </td>

      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="h-64 w-full min-w-[900px]">
        ...
        {coords.map((c) => (
          <g key={c.label}>
            ...
            <text x={c.x} y={c.y - 8} textAnchor="middle" fontSize="10" fill="#0f172a">{c.value.toFixed(1)}</text>
...
export default function OverviewPage({ rows }: { rows: Row[] }) {
  ...
  const forecastVsPosMonthly = useMemo(() => {
    const now = new Date();
    ...
    withStatus.forEach((r) => {
      ...
      if (forecast == null || pos == null || forecast < nowMonthStart) return;
  ...
  return (
    <>
      ...
      <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        {analysisRows.map((r) => (
          <div key={r.key} className="mb-2 flex items-center justify-between text-sm">
            <span>{r.key}</span>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-2 text-sm font-semibold">Monthly avg (Forecast Production Date - Purchase Order Sent)</div>
          {forecastVsPosMonthly.length === 0 ? <div className="text-sm text-slate-500">No non-finished rows from current month onward.</div> : <LineChart points={forecastVsPosMonthly} color="#047857" />}
        </div>
      </div>
    </>
  );

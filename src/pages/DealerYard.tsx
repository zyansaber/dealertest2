  ...
  const kpiTrendData = useMemo(() => {
    ...
    if (kpiTrendMode === "monthly") {
      ...
      return months.map((month) => ({
        ...
        handovers: handoverList.filter((x) => {
          const d = parseHandoverDate(x.handoverAt);
          return d && d >= month.start && d < month.end && x.dealerSlugFromRec === dealerSlug;
        }).length,
      }));
    }
    ...
    return weeks.map((week) => ({
      ...
      handovers: handoverList.filter((x) => {
        const d = parseHandoverDate(x.handoverAt);
        return d && d >= week.start && d < week.end && x.dealerSlugFromRec === dealerSlug;
      }).length,
    }));
            ...
            <Card className="border-slate-200 shadow-sm">
              ...
              <CardContent>
                ...
                <div className="mt-4 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">
                  <strong>Secondhand logic:</strong> counted from handover records where chassis matches pattern
                  <code className="mx-1">{"^[LNS][A-Z]{2}(23|24|25)\\d+$"}</code>
                  in current month.
                </div>
              </CardContent>
            </Card>
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={kpiTrendData}>
                      ...
                      <Bar dataKey="handovers" name="Handovers" fill="#8b5cf6" />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                      ...
                      <TableBody>
                        {handoverHistory.map((row) => (
                          <TableRow key={`kpi-${row.chassis}-${row.handoverAt || "unknown"}`}>
                            ...
                            <TableCell>{isSecondhandChassis(row.chassis) ? "Yes" : "No"}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>

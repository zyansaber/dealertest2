...
export default function OverviewPage({ rows, lang }: { rows: Row[]; lang: PlanningLang }) {
  ...
  const withStatus = useMemo(
    () =>
      rows.map((r) => {
        ...
        const rawModel = String((r.schedule as any)?.Model ?? "");
  ...
  const forecastVsSignedMonthlyWithMelbourneDays = useMemo<SplitTrendPoint[]>(() => {
    ...
    const rangeEnd = new Date(currentMonthStart.getFullYear(), currentMonthStart.getMonth() + 5, 1);
    ...
    filteredByModel.forEach((r) => {
      ...
      if (forecast == null || signed == null || leftPort == null) return;

      const leftPortDate = new Date(leftPort);
      const leftPortMonth = new Date(leftPortDate.getFullYear(), leftPortDate.getMonth(), 1);
      if (leftPortMonth < rangeStart || leftPortMonth > rangeEnd) return;

      const key = buildMonthKey(leftPortMonth);
      if (!buckets[key]) buckets[key] = { customerSum: 0, customerCount: 0, stockSum: 0, stockCount: 0 };
    ...
    while (cursor <= rangeEnd) {
      monthsInRange.push(buildMonthKey(cursor));
      cursor.setMonth(cursor.getMonth() + 1);
    }

    return monthsInRange.map((month) => {
      const v = buckets[month];
      return {
        label: month,
        customer: v?.customerCount ? Number((v.customerSum / v.customerCount).toFixed(2)) : null,
        stock: v?.stockCount ? Number((v.stockSum / v.stockCount).toFixed(2)) : null,

...
const pickScheduleCustomer = (source: Record<string, any> | undefined) => {
  if (!source) return { value: "", field: "" };
  const candidates: Array<[string, any]> = [
    ["Customer", (source as any)?.Customer],
    ["customer", (source as any)?.customer],
    ["customerName", (source as any)?.customerName],
    ["billToNameFinal", (source as any)?.billToNameFinal],
    ["billToParty", (source as any)?.billToParty],
  ];
  const hit = candidates.find(([, value]) => toStr(value));
  return hit ? { value: toStr(hit[1]), field: hit[0] } : { value: "", field: "" };
};
...
export default function OverallDashboardAdmin() {
  ...
  useEffect(() => {
    ...
    const unsubSchedule = subscribeToSchedule((data) => setOrders(Array.isArray(data) ? data : []), scheduleOptions);
    const unsubSchedule2024 = subscribeToSchedule2024((data) => setOrders2024(Array.isArray(data) ? data : []), scheduleOptions);
    const unsubCampervan = subscribeToCampervanSchedule((data) => setCampervanSchedule(Array.isArray(data) ? data : []));
  ...
  const { scheduleByChassis, scheduleByChassisLoose } = useMemo(() => {
    ...
    [...orders, ...orders2024].forEach((order) => {
    ...
    (campervanSchedule || []).forEach((item) => {
      register((item as any)?.chassisNumber, item);
      register((item as any)?.chassis, item);
    });

    return { scheduleByChassis: map, scheduleByChassisLoose: looseMap };
  }, [campervanSchedule, orders, orders2024]);
  ...
  const outlookRows = useMemo(() => {
    ...
    const inferYardType = (record: Record<string, any>, scheduleMatch?: ScheduleItem) => {
      const customer = pickScheduleCustomer(scheduleMatch as any).value || toStr(record?.customer);
    ...
    Object.entries(handoverAll || {}).forEach(([dealerSlug, rows]) => {
      ...
      Object.entries(rows || {}).forEach(([key, payload]) => {
        ...
        const scheduleCustomer = pickScheduleCustomer(scheduleMatch as any).value;
        const isStockType = isStockCustomer(scheduleCustomer);
  ...
  const srh16FrankstonHandoverDebugRows = useMemo(() => {
    ...
    Object.entries(handoverAll?.frankston || {}).forEach(([key, payload]) => {
      ...
      const customerPicked = pickScheduleCustomer(scheduleMatch as any);
      const customer = customerPicked.value;
      rows.push({
        dealer: "frankston",
        model,
        chassis: chassis || key,
        ...
        customerSourceField: customerPicked.field || "(none)",
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-md border bg-slate-50 p-3 text-sm text-slate-700">
              ...
              requiredBy6m、handover6mStock、handover6mMultiplier、requiredBy3m、handover3mStock、handover3mMultiplier（handover6mStock/3mStock按Stock口径计数；下方会列出 SRH16+frankston 每个chassis对应的schedule customer并标记Stock/Customer供检测）。
            </div>
            ...
            <div className="rounded-md border bg-amber-50 p-3 text-sm text-amber-900">
              ...
              注：schedule匹配使用 includeFinished/includeNoCustomer/includeNoChassis 全量索引，避免因筛选导致 customer 丢失。
            </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Customer Field</TableHead>
                ...
                {srh16FrankstonHandoverDebugRows.length === 0 ? (
                  ...
                  srh16FrankstonHandoverDebugRows.map((row, idx) => (
                    <TableRow key={`${row.chassis}-${idx}`}>
                      ...
                      <TableCell>{row.customerSourceField}</TableCell>

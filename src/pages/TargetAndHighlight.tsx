...
export default function TargetAndHighlight({ withSidebar = true }: { withSidebar?: boolean }) {
  ...
  const tierRules = useMemo(() => (tierConfig?.rules || {}) as Record<string, any>, [tierConfig]);
  ...
  const gapRows = useMemo(() => {
    ...
    return Array.from(map.values())
      .map((row) => {
        const tier = String(modelTierAssignments[row.model] || "").toLowerCase();
        if (!tier) return null;
        const multiplier = toNum(tierRules?.[tier]?.handover6mMultiplier);
        const yardIncoming = row.yard + row.incoming;
        const requiredBy6m = row.handover6mStock * multiplier;
      ...
      .filter((row): row is GapRow => Boolean(row) && row.gapBy6m > 0)
      .sort((a, b) => b.gapBy6m - a.gapBy6m);
  }, [handoverAll, modelTierAssignments, monthBuckets, schedule, tierRules, yardStockAll]);

  return (
    <div className={`min-h-screen bg-slate-50 ${withSidebar ? "" : "p-6"}`}>
      <div className="mx-auto max-w-7xl space-y-4">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  Tier Gap (All tiers) = requiredBy6m - (Yard + Incoming)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Table>

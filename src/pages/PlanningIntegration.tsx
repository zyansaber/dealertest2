...
export default function PlanningIntegration() {
  ...
  const weeklyNodeTrend = useMemo(() => {
    ...
    const weekSnapshots = weeks.map((week) => {
      const counts: Record<string, number> = {};
      const stateByChassis: Record<string, string> = {};
      ...
      rows.forEach((row) => {
        ...
        if (trackedMilestonesForWeekly.includes(lastKey as (typeof trackedMilestonesForWeekly)[number])) {
          counts[lastKey] += 1;
          stateByChassis[row.chassis] = lastKey;
        }
      });

      return { week, counts, stateByChassis };
    });
    ...
    return weekSnapshots.map((snapshot, index) => {
      ...
      const entered: Record<string, number | null> = {};
      const exited: Record<string, number | null> = {};

      trackedMilestonesForWeekly.forEach((key) => {
        increments[key] = prev ? snapshot.counts[key] - prev.counts[key] : null;
        entered[key] = prev ? 0 : null;
        exited[key] = prev ? 0 : null;
      });

      if (prev) {
        const chassisSet = new Set<string>([
          ...Object.keys(prev.stateByChassis),
          ...Object.keys(snapshot.stateByChassis),
        ]);

        chassisSet.forEach((chassis) => {
          const prevState = prev.stateByChassis[chassis];
          const currentState = snapshot.stateByChassis[chassis];
          if (prevState === currentState) return;

          if (currentState && trackedMilestonesForWeekly.includes(currentState as (typeof trackedMilestonesForWeekly)[number])) {
            entered[currentState] = (entered[currentState] ?? 0) + 1;
          }
          if (prevState && trackedMilestonesForWeekly.includes(prevState as (typeof trackedMilestonesForWeekly)[number])) {
            exited[prevState] = (exited[prevState] ?? 0) + 1;
          }
        });
      }
          ...
          <>
            <div className="mb-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-1 text-sm text-slate-600">
                Last 10-week chassis state trend including net changes, entered (in), and exited (out) for nodes from Purchase Order Sent to melbournePortDate.
              </p>
            </div>
                ...
                <thead className="bg-slate-100">
                  <tr>
                    ...
                    {trackedMilestonesForWeekly.map((key) => (
                      <th key={key} className="px-3 py-3 text-left font-semibold text-slate-700">{key} <span className="text-xs text-slate-500">(count / Δ / in / out)</span></th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {weeklyNodeTrend.map((week, rowIndex) => (
                    <tr key={week.weekLabel} className={rowIndex % 2 === 0 ? "bg-white" : "bg-slate-50/60"}>
                      ...
                      {trackedMilestonesForWeekly.map((key) => {
                        ...
                        const entered = week.entered[key];
                        const exited = week.exited[key];
                        const incText = inc == null ? "-" : inc >= 0 ? `+${inc}` : String(inc);
                        const inText = entered == null ? "-" : `+${entered}`;
                        const outText = exited == null ? "-" : `-${exited}`;
                        return (
                          <td key={`${week.weekLabel}-${key}`} className="whitespace-nowrap px-3 py-2.5 text-slate-700">
                            {week.counts[key]} <span className="text-xs text-slate-500">({incText} / {inText} / {outText})</span>
                          </td>
                        );
                      })}
                    </tr>

...
export default function OverallDashboardAdmin() {
  ...
  useEffect(() => {
    ...
    const unsubTier = subscribeOverallDashboardTierPlannerConfig((data) => {
      setTierConfig((prev) => ({
        ...prev,
        ...data,
        selectedTier: (data?.selectedTier as TierKey) || prev.selectedTier || "tier1",
        rules: {
          ...(prev.rules || {}),
          ...(data?.rules || {}),
        },
  ...
  const selectedTier = (tierConfig.selectedTier || "tier1") as TierKey;
  ...
  }, [activeRule, outlookRows, selectedTier]);
  ...
  const handleSaveTierPlanner = async () => {
    try {
      ...
      toast.success("Tier planner saved to Firebase");
    } catch (error) {
      console.error(error);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <div className="flex items-end">
                <Button onClick={handleSaveTierPlanner} disabled={savingTier} className="w-full">
                  {savingTier ? "Saving..." : "Save Tier Rules"}
                </Button>
              </div>
            </div>

            <div className="rounded-md border bg-slate-50 p-3 text-sm text-slate-700">
              Rule: list dealer/model when <strong>Yard + Incoming</strong> is smaller than the selected multiplier target.
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <Badge variant="secondary">Top stock/customer model by 6m/12m shown below</Badge>
            </div>

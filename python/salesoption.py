...
def build_schedulingvanoptions_df(df_schedule: pd.DataFrame, df_pgirecord: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["chassis", "pgidate", "wholesalepo", "vin_number", "vbeln"]:
        if c not in pgi.columns:
            pgi[c] = None
    pgi = pgi[["chassis", "pgidate", "wholesalepo", "vin_number", "vbeln"]].rename(
        columns={"pgidate": "from_pgidate", "vbeln": "salesOrder_pgirecord"}
    )

    s = s.merge(pgi, how="left", on="chassis")
    s["salesOrder"] = s.get("Sales Order Number")
    s["salesOrder"] = s["salesOrder"].where(s["salesOrder"].notna(), s["salesOrder_pgirecord"])
    ...
    out = pd.DataFrame({
        ...
        "model": s.get("Model"),
        "customer": s.get("Customer").fillna("Stock").astype(str),
        "from_pgidate": s["from_pgidate"].apply(lambda x: x if pd.notna(x) else None),
        "receivedAt": s.get("Order Received Date").apply(to_iso_utc_z),
...
def main():
    ...
    try:
        ...
        df_scheduling_van_options = build_schedulingvanoptions_df(df_schedule, df_pgirecord)

  ...
  return (
    <>
      <div className="mb-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="text-sm text-slate-600">Shared target saved to database. Input allowed until 2026-12.</p>
      </div>
      ...
      <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm overflow-auto">
        <table className="min-w-[1000px] text-sm">
          <thead>
            <tr>
              <th className="px-2 py-2 text-left">Shared Target</th>
              {monthsForTargetInput.map((m) => <th key={m} className="px-2 py-2 text-left">{m}</th>)}
            </tr>
          </thead>
          <tbody>
            <tr className="border-t">
              <td className="px-2 py-2 font-medium">All Metrics</td>
              {monthsForTargetInput.map((month) => (
                <td key={month} className="px-2 py-2">
                  <input
                    type="number"
                    className="w-24 rounded border border-slate-300 bg-slate-50 px-2 py-1"
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2">
        <Card className="border-slate-200">
          <CardHeader>
            <CardTitle className="text-sm">Total Difference (Actual - Shared Target)</CardTitle>
          </CardHeader>
          <CardContent>
            <p className={`text-4xl font-bold tracking-tight ${totalDifference >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{totalDifference}</p>
            <p className="text-xs text-slate-500">Selected metric only · 2025-06 to previous month</p>
          </CardContent>
        </Card>

        <Card className="border-slate-200">
          <CardHeader>
            <CardTitle className="text-sm">Difference Metric</CardTitle>
          </CardHeader>
          <CardContent>
            <select value={metric} onChange={(e) => setMetric(e.target.value as (typeof trackedMilestones)[number])} className="rounded border border-slate-300 bg-white px-3 py-2 text-sm">
              {trackedMilestones.map((m) => <option key={m} value={m}>{m}</option>)}
            </select>
          </CardContent>
        </Card>
      </div>
        ...
        {series.map((s) => (
          <div key={s.month} className="mb-2 flex items-center gap-3">
            ...
            <div className="h-4 flex-1 rounded bg-slate-100">
              <div className={`h-4 rounded ${s.value >= 0 ? "bg-emerald-600" : "bg-rose-600"}`} style={{ width: `${(Math.abs(s.value) / maxAbs) * 100}%` }} />
            </div>
            <div className="w-16 text-right text-sm font-semibold">{s.value}</div>
          </div>
        ))}
      </div>
    </>

  ...
  const waitingForSending = useMemo(() => {
    ...
    return withStatus
      ...
      .map((r) => {
        ...
        const hasPrice = Number.isFinite(Number(waitingOrderPrices[r.chassis]));
        const ready = hasPrice && forecastTs != null && forecastTs >= now && forecastTs <= in180;
        return { ...r, forecastTs, hasPrice, ready };
      });
  ...
  return (
    <>
      ...
      <div className="mb-4 rounded-xl border border-slate-200 bg-white shadow-sm overflow-auto">
        <table className="min-w-[1000px] divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-100">
            <tr>
              <th className="px-3 py-3 text-left">Chassis</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {withStatus.map((r) => (
              <tr key={`wfo-${r.chassis}`}>
                <td className="px-3 py-2.5">{r.chassis}</td>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-auto">
        <div className="border-b border-slate-200 px-4 py-3 text-sm font-semibold">waiting for sending</div>
        <table className="min-w-[1000px] divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-100">
            <tr>
              <th className="px-3 py-3 text-left">Chassis</th>
              <th className="px-3 py-3 text-left">Forecast Production Date</th>
              <th className="px-3 py-3 text-left">Price Filled</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {waitingForSending.map((r) => (
              <tr key={`wfs-${r.chassis}`} className={r.ready ? "animate-pulse bg-emerald-50" : ""}>
                <td className="px-3 py-2.5">{r.chassis}</td>
                <td className="px-3 py-2.5">{displayValue((r.schedule as any)?.["Forecast Production Date"])}</td>
                <td className="px-3 py-2.5">{r.hasPrice ? "Yes" : "No"}</td>
              </tr>
            ))}
          </tbody>
        </table>

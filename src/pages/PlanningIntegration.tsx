    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <main className={`min-w-0 p-6 transition-all ${collapsed ? "ml-20" : "ml-72"}`}>
        <Routes>
          ...
          <Route
            path="/waiting-for-po"
            element={<WaitingForOrderingPage withStatus={data.withStatus} waitingOrderPrices={data.waitingOrderPrices} saveWaitingPrice={data.saveWaitingPrice} />}
          />
          <Route path="/waiting-for-ordering" element={<Navigate to="/planningintegration/waiting-for-po" replace />} />
          <Route
            path="/target"
            element={<TargetPage monthsForTargetInput={data.monthsForTargetInput} monthsForDiff={data.monthsForDiff} targets={data.targets} saveSharedTarget={data.saveSharedTarget} monthlyActuals={data.monthlyActuals} />}
          />
          <Route path="/report" element={<ReportPage trend={data.trend} granularity={reportGranularity} setGranularity={setReportGranularity} />} />

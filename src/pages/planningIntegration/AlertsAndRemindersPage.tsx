...
export default function AlertsAndRemindersPage({ rows, lang }: { rows: Row[]; lang: PlanningLang }) {
  ...
  const reminders = useMemo(() => {
    ...
    return rows
      .map((row) => {
        ...
        const gapReasons = alertRules
          .slice(0, 3)
          .map((rule) => {
            const reason = delayReasonMap[`${rule.id}__${chassis}`] || "-";
            return `${lang === "zh" ? rule.titleZh : rule.titleEn}: ${reason}`;
          })
          .join(" | ");
  ...
  const downloadRemindersExcel = () => {
    const data = reminders.map((item) => ({
      ...
      Timeline: reminderTimeline.join(" -> "),
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-6">
            <button
              ...
              className={`rounded-xl border p-3 text-left ${selectedRule === "all" ? "border-slate-700 bg-slate-900 text-white" : "border-slate-200 bg-white"}`}
            >
            ...
            {alertRules.map((rule) => (
              <button
                ...
                className={`rounded-xl border p-3 text-left ${selectedRule === rule.id ? "border-rose-500 bg-rose-50" : "border-rose-100 bg-white hover:bg-rose-50"}`}
              >
                      ...
                      return (
                        <tr key={item.reasonKey} className={`border-t border-slate-100 align-top ${customerRowClass}`}>
                          ...
                          <td className="px-3 py-2">
                            <span className={`rounded-full px-2 py-0.5 text-xs font-semibold uppercase ${item.type === "customer" ? "bg-amber-200/70 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{item.type}</span>
                          </td>
                      ...
                      const mainRow = (
                        <tr key={`${item.chassis}-${item.requestTs}`} className={`border-t border-slate-100 ${customerRowClass}`}>
                          ...
                          <td className={`px-3 py-2 font-medium ${item.overdueDays > 0 ? "text-rose-600" : "text-amber-700"}`}>
                            {item.overdueDays > 0
                              ? `${tr(lang, "Delayed", "已延迟")} ${item.overdueDays}${tr(lang, " days", "天")}`
                              : `${tr(lang, "Remaining", "剩余")} ${item.remainingDays}${tr(lang, " days", "天")}`}
                          </td>
                        </tr>
                      );
                          ...
                          <td colSpan={8} className="px-3 pb-3 pt-1">
                            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-xs text-slate-700">
                              <div className="mb-1">
                                ...
                                <span>{reminderTimeline.join(" → ")}</span>
                              </div>
                              <div className="mb-2">
                                <span className="font-semibold">{tr(lang, "Gap Reasons", "差异原因")}：</span>
                                <span>{item.gapReasons}</span>
                              </div>
                              <div className="flex items-center gap-2">
                                ...
                                <input
                                  type="date"
                                  value={etaValue}
                                  onChange={(e) => setEditingEta((prev) => ({ ...prev, [item.chassis]: e.target.value }))}

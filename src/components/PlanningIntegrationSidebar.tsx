import { BarChart3, ClipboardList, Ship, Target } from "lucide-react";
...
export default function PlanningIntegrationSidebar({ activeTab, onTabChange, totalRows }: PlanningIntegrationSidebarProps) {
  return (
    <aside className="fixed left-0 top-0 flex h-screen w-72 shrink-0 flex-col border-r border-slate-800 bg-slate-950 text-slate-100">
      ...
      <div className="px-4 py-4">
        <div className="rounded-lg border border-slate-700 bg-slate-900 p-4 space-y-2">
          ...
          <div className="text-xs text-slate-400">schedule rows (ID ordered)</div>
        </div>
      </div>
    </aside>
  );

...
export default function AustraliaDealerMap({
  ...
  useEffect(() => {
    setFrameIndex(Math.max(0, activeFrames.length - 1));
    setAnimating(false);
    setHasPlayed(false);
  }, [activeFrames.length, metricFilter]);
  ...
  }, [activeFrames, metricStateTotals, modelRangeSet]);
  ...
  const latestFrameLabel = activeFrames[Math.max(0, activeFrames.length - 1)]?.label;
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              <div className="mt-3 max-h-[320px] space-y-2 overflow-auto pr-1">
                {modelRangeMetrics.map((row) => {
                  const metricValue = metricFilter === "orders" ? row.orders : row.forecast;
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            setSelectedModelRanges((prev) => {
                              ...
                              if (enabled) {
                                if (base.includes(row.modelRange)) return prev;
                                return [...base, row.modelRange];
                              }
                              const next = base.filter((range) => range !== row.modelRange);
                              return next.length === modelRangeOptions.length ? [] : next;
                            });
                          }}

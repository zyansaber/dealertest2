 ...
 useEffect(() => {
  ...
  const kpiTrendData = useMemo(() => {
    ...
    if (kpiTrendMode === "monthly") {
      ...
      return months.map((month) => ({
        ...
        handoversCustomer: handoverList.filter((x) => {
          const d = parseHandoverDate(x.handoverAt);
          return d && d >= month.start && d < month.end && x.dealerSlugFromRec === dealerSlug && x.type === "Customer";
        }).length,
      }));
    }
    ...
    return weeks.map((week) => ({
      ...
      handoversCustomer: handoverList.filter((x) => {
        const d = parseHandoverDate(x.handoverAt);
        return d && d >= week.start && d < week.end && x.dealerSlugFromRec === dealerSlug && x.type === "Customer";
      }).length,
    }));
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={kpiTrendData}>
                      ...
                      <Bar dataKey="handoversStock" name="Handovers (Stock)" stackId="handovers" fill="#f59e0b" />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>

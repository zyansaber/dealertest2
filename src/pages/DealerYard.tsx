...
export default function DealerYard() {
 ...
 useEffect(() => {
  ...
  // KPI calculations using current month range
    ...
    () =>
      handoverList.filter(
        (x) =>
          ...
          isDateWithinRange(parseHandoverDate(x.handoverAt), currentMonthStart, currentMonthEnd)
      ).length,
  ...
  const selectedTrendHandoverRows = useMemo(() => {
    if (!selectedTrendLabel) return [] as typeof handoverHistory;
    const now = new Date();
    if (kpiTrendMode === "monthly") {
      ...
      if (!target) return [];
      return handoverHistory.filter((row) => {
        const d = parseHandoverDate(row.handoverAt);
  ...
  }, [selectedTrendLabel, kpiTrendMode, handoverHistory]);
  ...
  const showExpandableDetails = PRICE_ENABLED_DEALERS.has(dealerSlug);
  ...
  const handleAddNewChassis = async () => {
    ...
    if (!chassis) {
      setAddStatus({ type: "err", msg: "Chassis is required." });
      return;
    }
        ...
        <DialogContent className="max-w-3xl">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm text-slate-600">
              Chassis is required. VIN Number, Model, Received At, AUD Price, Type, and Days In Yard are optional.
            </DialogDescription>
          </DialogHeader>
            ...
            {selectedTrendHandoverRows.length === 0 ? (
              ...
              <div className="max-h-[420px] overflow-auto rounded border">
                <Table>
                  ...
                  <TableBody>
                    {selectedTrendHandoverRows.map((row) => (
                      <TableRow key={`trend-${row.chassis}-${row.handoverAt || "unknown"}`}>
                        <TableCell>{formatDateTime(row.handoverAt)}</TableCell>
                        <TableCell>{row.chassis}</TableCell>
                        <TableCell>{row.type}</TableCell>
                        <TableCell>{isSecondhandByMaterialCode(row.materialCode) ? "Yes" : "No"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          <Card className="border-slate-200 shadow-sm hover:shadow-md transition">
            ...
            <CardContent>
              {waitingForReceiving.length === 0 ? (
                <div className="text-sm text-slate-500">No PGI records awaiting receipt.</div>
              ) : (
        ...
        {activeTab === "yard" && (
          <>
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 items-stretch">
              <div className="xl:col-span-2 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-2 gap-3">
                <Card className="border-slate-200 shadow-sm hover:shadow-md transition">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Second Hand</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="text-4xl font-extrabold tracking-tight text-amber-600">{vanConditionCounts.secondHand}</div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-slate-200 shadow-sm hover:shadow-md transition">
                  ...
                  <CardContent>
                    <div className="text-4xl font-extrabold tracking-tight text-sky-600">{vanConditionCounts.newVans}</div>
                  </CardContent>
                </Card>

                <Card className="border-slate-200 shadow-sm hover:shadow-md transition sm:col-span-2 xl:col-span-2">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Customer Type</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="text-4xl font-extrabold tracking-tight text-emerald-600">{kpiYardStockCurrent.customer}</div>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card className="border-slate-200 shadow-sm hover:shadow-md transition xl:col-span-6">
                <CardHeader className="flex items-center justify-between">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="font-semibold">Model</TableHead>
                          {showPriceColumn && <TableHead className="font-semibold">Wholesale (excl. GST)</TableHead>}
                          {showPriceColumn && <TableHead className="font-semibold">Retail (incl. GST)</TableHead>}
                          {showPriceColumn && <TableHead className="font-semibold">Discount (incl. GST)</TableHead>}
                          <TableHead className="font-semibold">Customer</TableHead>
                      ...
                      <TableBody>
                        {pendingYardList.length > 0 && (
                          <TableRow className="bg-amber-50/60">
                            <TableCell colSpan={showPriceColumn ? 13 : 10} className="text-sm font-semibold text-amber-700">
                              Pending Into Yard
                            </TableCell>
                          </TableRow>
                        )}
                        {pendingYardList.map((row) => (
                          <TableRow key={`pending-${row.chassis}`} className="bg-amber-50/40">
                            ...
                            {showPriceColumn && <TableCell>-</TableCell>}
                            {showPriceColumn && <TableCell>-</TableCell>}
                            <TableCell>{toStr(row.customer) || "-"}</TableCell>
                            <TableCell>
                        ...
                        {yardListDisplay.map((row) => (
                          <Fragment key={row.chassis}>
                            <TableRow className={showExpandableDetails && expandedChassis[row.chassis] ? "bg-slate-50/80" : ""}>
                              <TableCell className="font-medium">
                                {showExpandableDetails ? (
                                  <button
                                    type="button"
                                    className="inline-flex items-center gap-2 rounded-md border border-slate-500 bg-slate-700 px-2 py-1 text-left text-white transition hover:border-slate-400 hover:bg-slate-600"
                                    onClick={() =>
                                  ...
                                  >
                                    ...
                                    <span className="rounded bg-slate-500 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-100">
                                      Details
                                    </span>
                                  </button>
                                ) : (
                              ...
                              <TableCell>{formatDateOnly(row.receivedAt)}</TableCell>
                              <TableCell>{toStr(row.model) || "-"}</TableCell>
                              {showPriceColumn && <TableCell className="font-medium text-slate-900">{row.wholesaleDisplay}</TableCell>}
                              {showPriceColumn && <TableCell className="font-medium text-emerald-800">{row.retailSalePriceDisplay}</TableCell>}
                              {showPriceColumn && <TableCell className="font-medium text-amber-800">{row.discountDisplay}</TableCell>}
                              <TableCell>{toStr(row.customer) || "-"}</TableCell>
                              <TableCell>
                            ...
                            {showExpandableDetails && expandedChassis[row.chassis] && (
                              <TableRow>
                                <TableCell colSpan={showPriceColumn ? 13 : 10} className="bg-slate-900/10 p-3">
                                  <div className="rounded-xl border-2 border-slate-700 bg-slate-950/5 p-4 shadow-md">
                                    ...
                                    <div className="mb-4 grid grid-cols-1 gap-3 text-sm md:grid-cols-3">
                                      <div className="rounded-lg border-2 border-slate-700 bg-slate-100 px-3 py-2">
                                      ...
                                      <div className="rounded-lg border-2 border-emerald-600 bg-emerald-100 px-3 py-2">
                                      ...
                                      <div className="rounded-lg border-2 border-amber-600 bg-amber-100 px-3 py-2">
                                    ...
                                    <div className="overflow-auto rounded-lg border-2 border-slate-700 bg-white">
                                      <Table>

    ...
    return weeks.map((week, index) => {
      ...
      rows.forEach((row) => {
        trackedMilestonesForWeekly.forEach((key) => {
          ...
          if (prevWeek && ts >= prevWeek.start && ts <= prevWeek.end) {
            prevCounts[key] += 1;
          }
        });
      });
      ...
      trackedMilestonesForWeekly.forEach((key) => {
        increments[key] = prevWeek ? counts[key] - prevCounts[key] : null;
      });
          ...
          <>
            <div className="mb-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-1 text-sm text-slate-600">
                Last 10-week event trend: how many milestone dates happened in each week (e.g. weekly Left Port count).
              </p>
            </div>
                ...
                <thead className="bg-slate-100">
                  <tr>
                    ...
                    {trackedMilestonesForWeekly.map((key) => (
                      <th key={key} className="px-3 py-3 text-left font-semibold text-slate-700">{key} <span className="text-xs text-slate-500">(weekly count / vs last week)</span></th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">

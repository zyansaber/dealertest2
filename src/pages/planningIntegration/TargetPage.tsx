...
export default function TargetPage({
  ...
  const [metric, setMetric] =
    useState<(typeof trackedMilestones)[number]>("leavingFactory");
  ...
  const differences = useMemo(() => {
    ...
    trackedMilestones.forEach((m) => {
      ...
      monthsForDiff.forEach((month) => {
        out[m][month] =
          (monthlyActuals[m]?.[month] ?? 0) - (targets[month] ?? 0);
      });
    });
    return out;
  }, [monthsForDiff, monthlyActuals, targets]);

  const series = monthsForDiff.map((month) => ({
    month,
    value: differences[metric][month] ?? 0,
  }));
  const totalDifference2026 = useMemo(
    () =>
      series
  ...
  const currentProgressRatio =
    currentMonthTarget > 0
      ? Math.min(currentMonthActual / currentMonthTarget, 1)
      : 0;
    ...
    <>
      <div className="mb-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="text-sm text-slate-600">
          {tr(
            lang,
            "Shared monthly target saved to Firebase (2025-06 ~ 2026-12).",
            "共享月度目标已保存到 Firebase（2025-06 ~ 2026-12）。",
          )}
        </p>
      </div>
          ...
          <thead className="bg-slate-900 text-white">
            <tr>
              <th className="sticky left-0 z-10 bg-slate-900 px-4 py-3 text-left font-semibold">
                {tr(lang, "Shared Target", "共享目标")}
              </th>
              {monthsForTargetInput.map((m) => (
                <th
                  key={metricText(lang, m)}
              ...
              {monthsForTargetInput.map((month) => (
                <td key={month} className="px-3 py-2">
                  <input
                    ...
                    onChange={(e) =>
                      saveSharedTarget(month, Number(e.target.value || 0))
                    }
                  />
                </td>
              ))}
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-2">
        <Card className="border-slate-200">
          <CardHeader>
            <CardTitle className="text-sm">
              {tr(
                lang,
                "Total Difference 2026 (Actual - Shared Target)",
                "2026年总差值（实际 - 共享目标）",
              )}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p
              className={`text-4xl font-bold tracking-tight ${totalDifference2026 >= 0 ? "text-emerald-700" : "text-rose-700"}`}
            >
              {totalDifference2026}
            </p>
            <p className="text-xs text-slate-500">
              {tr(
                lang,
                "Selected metric only · Jan 2026 to Dec 2026",
                "仅当前指标 · 2026年1月到12月",
              )}
            </p>
          </CardContent>
        ...
        <Card className="border-slate-200">
          <CardHeader>
            <CardTitle className="text-sm">
              {tr(lang, "Difference Metric", "差值指标")}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <select
              value={metric}
              onChange={(e) =>
                setMetric(e.target.value as (typeof trackedMilestones)[number])
              }
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 text-sm font-semibold">
          {tr(lang, "Difference Bar Chart", "差值柱状图")}：
          {metricText(lang, metric)}（
          {tr(lang, "Actual - Shared Target", "实际 - 共享目标")}）
        </div>
        <div className="mb-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
          <div className="mb-1 text-xs font-semibold text-slate-700">
            {tr(lang, "Current month progress", "当月进度")} ·{" "}
            {currentMonthLabel}
          </div>
          ...
          <div className="text-xs text-slate-700">
            {tr(lang, "Current", "当前")}:{" "}
            <span className="font-semibold">{currentMonthActual}</span> /
            {tr(lang, "Target", "目标")}:{" "}
            <span className="font-semibold">{currentMonthTarget}</span>
            {currentRemain > 0 ? (
              <span className="ml-2 text-amber-700">
                ({tr(lang, "Remaining", "还差")} {currentRemain})
              </span>
            ) : null}
            {currentOver > 0 ? (
        ...
        {series.map((s) => (
          <div key={s.month} className="mb-2 flex items-center gap-3">
            ...
            <div
              className={`w-16 text-right text-sm font-semibold ${s.value >= 0 ? "text-emerald-700" : "text-rose-700"}`}
            >
              {s.value}
            </div>
          </div>

...
export default function OverallDashboardProfitPage() {
  ...
  const summary = useMemo(() => {
    ...
    chassisPrices.forEach((row) => {
      ...
      // Forecast profit: MUST be customer + hasInvoice=false
      const isCustomer = normalizeText(row.billToNameFinal) !== "stock";
      const isUninvoiced = row.hasInvoice === false;
      const forecastDate = parseDate(String(scheduleMatch?.["Forecast Production Date"] ?? ""));
      if (!isCustomer || !isUninvoiced || !forecastDate) return;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Forecast Profit (+30 days, customer, hasInvoice=false)</CardTitle>
          </CardHeader>

...
export default function OverallDashboardAdmin() {
  ...
  const { scheduleByChassis, scheduleByChassisLoose } = useMemo(() => {
    ...
    orders.forEach((order) => {
      ...
      const loose = normalizeChassisLoose(order?.Chassis);
  ...
  const outlookRows = useMemo(() => {
    ...
    Object.entries(yardStockAll || {}).forEach(([dealerSlug, stockRows]) => {
      Object.entries(stockRows || {}).forEach(([chassis, payload]) => {
        ...
        const scheduleMatch = scheduleByChassis[normalizeChassis(chassis)] || scheduleByChassisLoose[normalizeChassisLoose(chassis)];
    ...
    Object.entries(handoverAll || {}).forEach(([dealerSlug, rows]) => {
      Object.entries(rows || {}).forEach(([key, payload]) => {
        ...
        const scheduleMatch = scheduleByChassis[chassis] || scheduleByChassisLoose[normalizeChassisLoose(chassis)];
        const model = normalizeModel(rec?.model || scheduleMatch?.Model);
        if (!model) return;
        const recDealer = normalizeDealerSlug(rec?.dealerSlug || rec?.dealerName || dealerSlug);
        const scheduleDealer = normalizeDealerSlug((scheduleMatch as any)?.Dealer);
        const handoverDealer = FACTORY_DEALER_SLUGS.has(recDealer)
          ? recDealer
          : FACTORY_DEALER_SLUGS.has(scheduleDealer)
            ? scheduleDealer
            : recDealer;
        if (!FACTORY_DEALER_SLUGS.has(handoverDealer)) return;
        const row = ensure(handoverDealer, model);
        // Stock-only handover counting: align with Stock Model Outlook bracket values.
        // Same rule as /overall-dashboard: classify stock via matched schedule customer.
        const isStockType = isStockCustomer((scheduleMatch as any)?.Customer);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-md border bg-slate-50 p-3 text-sm text-slate-700">
              ...
              requiredBy6m、handover6mStock、handover6mMultiplier、requiredBy3m、handover3mStock、handover3mMultiplier（handover口径=Stock Model Outlook括号里的stock数量）。
            </div>
